...
- **`QLBH.DTO`**: Data Transfer Object. Chứa các đối tượng truyền dữ liệu.
- **`QLBH.Web`**: Ứng dụng Frontend (Web/MVC).
...
2. Đổi Default Project trong Package Manager Console là ```QLBH.DAL```và cài đặt Northwind ( xài cho project ) bằng ```Update-Database -Context QLBH_DBContext```
3. Build solution: 
   ```bash
   dotnet build
   ```
